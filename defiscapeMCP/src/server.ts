...
/**
 * 3) Resource ─ アクティブなクロスチェーンオーダー情報
 *    URI 例: fusion-orders://active → 利用可能なオーダーリスト
 */
server.registerResource(
  "fusion-orders",
...
server.registerTool(
  ...
  async (input) => {
    try {
      ...
      // 2. アクティブオーダーリストから該当オーダーを検索してチェーンIDを取得
      ...
      // 3. オーダー詳細から見積もりパラメータを構築（アクティブオーダーからチェーンIDを取得）
      const order = orderStatus.order;
      const quoteParams = {
        srcChainId: activeOrder.srcChainId,
        dstChainId: activeOrder.dstChainId,
        srcTokenAddress: order.makerAsset, // リゾルバーが受け取るトークン（元のtakerAsset）
        dstTokenAddress: order.takerAsset, // リゾルバーが提供するトークン（元のmakerAsset）
        amount: order.makingAmount, // リゾルバーが受け取る量
      ...
      // Gas estimate from preset data instead of quote.estimate
      const gasEstimate = presetDetails?.costInDstToken ? Number(presetDetails.costInDstToken) : 0;
      const gasCostWei = BigInt(gasEstimate) * BigInt(maxGasPrice ? maxGasPrice * 1e9 : 20e9); // デフォルト20 Gwei
      const gasCostEth = Number(gasCostWei) / 1e18;
      
      // 6. トークン価格とプロフィット計算（簡略化）
      const inputAmount = Number(order.makingAmount) / 1e18; // 受け取る量
      const outputAmount = Number(quote.dstTokenAmount) / 1e18; // 提供する量
